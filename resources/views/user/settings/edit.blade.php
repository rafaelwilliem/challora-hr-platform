...
<div class="edit-container">
    ...
    <form method="post" action="{{ route('user.settings.update') }}">
                ...
                @foreach($user->workExperiences as $exp)
                    <div class="dynamic-list-item flex-col items-start gap-4 mb-6">
                        <div class="grid grid-cols-2 gap-4 w-full">
                            <input type="text" name="work_title[]" class="form-input" placeholder="Posisi" value="{{ $exp->title }}">
                            <input type="text" name="work_company[]" class="form-input" placeholder="Perusahaan" value="{{ $exp->company_name }}">
                        </div>
                        <div class="flex gap-4 w-full">
                            <input type="text" name="work_year_start[]" class="form-input" placeholder="Tahun Mulai" value="{{ $exp->year_start }}">
                            <input type="text" name="work_year_end[]" class="form-input" placeholder="Tahun Selesai" value="{{ $exp->year_end }}">
                        ...
                        <textarea name="work_description[]" class="form-textarea" placeholder="Deskripsi kerja...">{{ $exp->description }}</textarea>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="flex gap-4 mt-12 pb-12">
            ...
            <a href="{{ route('jobs.index') }}" class="btn-cancel">Kembali</a>
        </div>
    </form>
